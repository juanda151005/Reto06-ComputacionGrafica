Shader "Basics/TessellatedWaves"
{
    Properties
    {
        ...
        _TessellationAmount("Tessellation Amount", Range(1, 64)) = 10
        _TessellationFadeStart("Tessellation Fade Start", Float) = 5
        _TessellationFadeEnd("Tessellation Fade End", Float) = 25
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Vertex Shader ------------------------------------------------
            // Pasa datos al Hull Shader (en world space)
            ...
            // Calcula cuánto subdividir cada triángulo según distancia a la cámara
            tessFactors patchConstantFunc(InputPatch<tessControlPoint, 3> patch)
            {
                ...
                float fadeDistN = _TessellationFadeEnd - _TessellationFadeStart;

                float edgeFactor0 = saturate(1.0f - (dist0 - _TessellationFadeStart) / fadeDistN);
                float edgeFactor1 = saturate(1.0f - (dist1 - _TessellationFadeStart) / fadeDistN);
                float edgeFactor2 = saturate(1.0f - (dist2 - _TessellationFadeStart) / fadeDistN);
            ...
            // Domain Shader -------------------------------------------------
            // Aquí se aplica el movimiento de olas a los vértices nuevos
            [domain("tri")]
